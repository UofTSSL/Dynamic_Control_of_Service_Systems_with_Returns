...
directory = "./noisy_case_trans_optimistic/"
log_file = "sims.log"
...
class Experiment:
    ...
    def __init__(self, **kwargs):
        ...
        y0s = np.linspace(0, (self.service_rate * self.server_num - self.arrival_rate) / self.return_rate, 200)
        ...
        self.policy_variants = [self.eq_policy, self.simple_policy, self.optimal_policy, self.safety_policy]
...
class Simulation:
    ...
    def Arrival(self):
        ...
        self.is_intervening_stat.Record(self.policy(self.X * 50 / self.experiment.scale, self.estimate * 50 / self.experiment.scale))
    ...
    def EndOfService(self):
        ...
        p = self.policy(self.X * 50 / self.experiment.scale, self.estimate * 50 / self.experiment.scale)
        ...
        if SimRNG.Uniform(0, 1, self.stream) < p:
            ...
            SimFunctions.SchedulePlus(self.calendar, "Return", SimRNG.TruncExpon(1/self.experiment.trunc_return_rate, 30, self.stream), p)
        else:
            self.Z += 1
            SimFunctions.SchedulePlus(self.calendar, "Expire", 30, p)
    ...
    def Return(self, p_at_discharge):
        self.orbit_stat.Record(self.Y)
        customer = SimClasses.Entity()
        self.queue.Add(customer)
    ...
    def Expire(self, p_at_discharge):
        self.Z -= 1
        self.estimate -= p_at_discharge
    ...
    def initialize(self):
        ...
        for j in range(self.Y):
            SimFunctions.SchedulePlus(self.calendar, "Return", SimRNG.TruncExpon(1/self.experiment.trunc_return_rate, 30, self.stream), 1.0)
    ...
    def simulate(self, stream=0):
        ...
        while next_event.EventType != "EndSimulation":
            if next_event.EventType == "Arrival":
                ...
                self.Return(next_event.WhichObject)
            elif next_event.EventType == "Expire":
                self.Expire(next_event.WhichObject)
...
def main():
    Ms = [1110] # [250, 500, 750, 1000]
    hs = [500, 1000, 1500, 2000, 2500, 3000] # [1000, 2000, 3000, 4000, 5000]

    inits = [(10,60), (60,10), (60,60)] # [(60, 40), (60, 5), (20, 40)]
